...
def openFile():
    ...
    try:
        tf = filedialog.askdirectory(
            initialdir="D:\SIT Year 2 Tri 2\2207\Assignment2\androidobsv1\Android-Obfuscator",
            title="Open Folder",
...
def locateDecompiledSmali(apkName):
    '''Get decompiled APK smali files'''
    return
...
def displayAllSmali(apkFolder):
    print("B")
    targetDir = apkFolder + "/smali/org/owasp/goatdroid/fourgoats/activities/"
    fileList = os.listdir(targetDir)
    ...
    for file in fileList:
        if (file.endswith(".smali")):
            ...
            #binding scrollbar with text area for frame 1
            ...
            # binding scrollbar with text area for frame 2
            ...
            with open(targetDir+file, "r", encoding="utf-8") as f:
                # prints to textbox
                file_cont = f.read()
                # go back to beginning of file and convert lines into a list
                f.seek(0)
                get_lines = list(f.readlines())
